...
def get_server(api):
    ...
    payload = "{\"conversion\":[{\"category\":\"audio\",\"target\":\"mp3\"}]}"
    headers = {
        'x-oc-api-key': api,
...
def upload_file(server_url, api):
    ...
    send_files = {'file': (''.join(random.choices(string.ascii_uppercase + string.digits, k=5)) + '.mp3',
                           music_mp3, 'audio/wav')}
...
def download_file(api, job_id, filename):
    ...
    result = filename.replace(".wav", ".mp3")
